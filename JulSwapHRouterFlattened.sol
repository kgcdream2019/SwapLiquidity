// File: contracts/huobi/interfaces/IJulSwapHPair.sol

// SPDX-License-Identifier: GPL-3.0-or-later

pragma solidity >=0.5.0;

interface IJulSwapHPair {
...
// File: contracts/huobi/libraries/JulSwapHLibrary.sol

pragma solidity >=0.5.0;

library JulSwapHLibrary {
    using SafeMathJulSwap for uint;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'JulSwapHLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'JulSwapHLibrary: ZERO_ADDRESS');
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IJulSwapHPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'JulSwapHLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'JulSwapHLibrary: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'JulSwapHLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'JulSwapHLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'JulSwapHLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'JulSwapHLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'JulSwapHLibrary: INVALID_PATH');
...
// File: contracts/huobi/libraries/TransferHelper.sol
...
// File: contracts/huobi/interfaces/IJulSwapHRouter01.sol

pragma solidity >=0.6.2;

interface IJulSwapHRouter01 {
...
// File: contracts/huobi/interfaces/IJulSwapHRouter02.sol

pragma solidity >=0.6.2;


interface IJulSwapHRouter02 is IJulSwapHRouter01 {
...
// File: contracts/huobi/interfaces/IWHT.sol
...
// File: contracts/huobi/JulSwapHRouter.sol
