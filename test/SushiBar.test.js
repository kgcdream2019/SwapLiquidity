...
contract('SwapLiquidityToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        ...
        this.bar = await SwapLiquidityBar.new(this.swapliquidity.address, { from: alice });
        this.swapliquidity.mint(alice, '100', { from: alice });
        this.swapliquidity.mint(bob, '100', { from: alice });
        this.swapliquidity.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.swapliquidity.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.swapliquidity.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.swapliquidity.approve(this.bar.address, '100', { from: alice });
        await this.swapliquidity.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.swapliquidity.balanceOf(this.bar.address)).valueOf(), '30');
        // SwapLiquidityBar get 20 more SLTs from an external source.
        await this.swapliquidity.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more SLTs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.swapliquidity.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.swapliquidity.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.swapliquidity.balanceOf(bob)).valueOf(), '98');
    });
});
